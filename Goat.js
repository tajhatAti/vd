...
(async () => {
	...
	let sendMail = async () => { console.log("[MAIL MOCK] Mail service not configured.") };
	...
	if (email && clientId && clientSecret && refreshToken) {
		...
		try {
			...
			utils.log.warn("MAIL", "Failed to initialize Gmail service (Token or OAuth error). Using dummy mail service.");
		}
	} else {
		utils.log.warn("MAIL", "Gmail API credentials not provided in config.json. Email notifications are disabled.");
	}
	...
	try {
		...
		utils.log.warn("VERSION CHECK", "Could not check for updates because of network restrictions.");
	}
	...
	if (utils.drive) {
		try {
			...
			utils.log.warn("GOOGLE DRIVE", "Failed to check or create GoatBot folder on Google Drive.");
		}
	}
	// ———————————————————— LOGIN ———————————————————— //
