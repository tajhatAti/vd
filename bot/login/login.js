...
async function startBot(loginWithEmail) {
	...
	try {
		...
		log.warn("VERSION", "Could not fetch tooOldVersions list. Skipping check.");
	}
	// nếu version cũ hơn
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
			...
			try {
				...
				log.warn('GBAN', "Could not check GBAN list due to network issues. Skipping.");
			}
			// ———————————————— NOTIFICATIONS ———————————————— //
			let notification;
			try {
				...
				log.warn("NOTI", "Could not get notifications due to network issues. Skipping.");
				notification = "";
			}
			...
			if (global.GoatBot.config.serverUptime.enable == true && !global.GoatBot.config.dashBoard?.enable && !global.serverUptimeRunning) {
				...
				let html = "<h1>GoatBot Uptime Server Active</h1>";
				try {
					const response = await axios.get("https://raw.githubusercontent.com/ntkhang03/resources-goat-bot/master/homepage/home.html");
					html = response.data;
				} catch (err) {
